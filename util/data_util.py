...
class NYBDataset(Dataset):
    def __init__(self, data_dir, usernum, order=0, model_name='LSTM'):
        ...
        self.raw_data = self.raw_data.drop_duplicates()  # 删除重复记录
        ...
        # 删除多余记录
        self.selected_data = self.raw_data.drop(
            self.raw_data[[False if i in self.BusList else True for i in self.raw_data.VehicleRef]].index)
        ...
        if model_name == 'LSTM':
            ...
            scaler = MinMaxScaler()
            self.selected_data[:, columns_to_normalize] = scaler.fit_transform(
                self.selected_data[:, columns_to_normalize])
...
class TokyoCheckinDataset(Dataset):
    def __init__(self):
        ...
        self.raw_data_pd = self.raw_data_pd.drop(
            self.raw_data_pd[[False if i == '822' else True for i in self.raw_data_pd[1]]].index)
...
class GowallaCheckinDataset(Dataset):
    def __init__(self):
        ...
        self.raw_data_pd = self.raw_data_pd.drop_duplicates()
        self.raw_data_pd = self.raw_data_pd.drop(
            self.raw_data_pd[[False if i == '10971' else True for i in self.raw_data_pd[1]]].index)
        print("data.shape: {}".format(self.raw_data_pd.shape))
