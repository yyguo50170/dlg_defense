...
def one_hot_encoding(raw_value, max_value, interval):
    print((raw_value / max_value) * interval % interval)
...
def location_time_embedding_single(location_code, time):
    time_code = np.array([])
    # time
    parsed_time = pd.to_datetime(time[0])
    # 提取时间信息
    year = parsed_time.year
    time_code = np.concatenate([time_code, one_hot_encoding(year - 2016, 2, 2)])
    month = parsed_time.month
    ...
    all_code = np.concatenate([location_code, time_code])
    return all_code
...
def inv_embedding_location(code_emb):
    res_list = []
    for i in range(len(code_emb)):
        res_list.append(inv_embedding_location_single(code_emb[i]))
    return np.array(res_list)


def inv_embedding_location_single(code_emb):
